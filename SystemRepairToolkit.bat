...
:: ============================================================================
:: ROBUST BATCH LAUNCHER FOR EMBEDDED POWERSHELL (FINAL TEMPLATE V2)
::
:: CRITICAL: This file MUST be saved with "ANSI" or "UTF-8" (NO BOM).
:: Use Notepad++ or VS Code to confirm encoding.
:: ============================================================================

echo.
echo =======================================================
echo  This script will execute an embedded PowerShell utility.
echo =======================================================
echo.
...
:: Find the line number of the PowerShell script start
for /f "delims=:" %%a in ('findstr /n /c:"::PS_SCRIPT_START" "%~f0"') do (
    set /a "start_line=%%a + 1"
)

:: Extract the PowerShell script into a temporary file
set "temp_ps=%temp%\SystemRepairToolkit_%RANDOM%.ps1"
more +%start_line% "%~f0" > "%temp_ps%"

:: Execute it with Bypass policy
...
echo.
echo [+] PowerShell session has ended.
pause
exit /b
